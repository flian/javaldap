 ...
 */
public org.javaldap.server.syntax.DirectoryString getSubject() {
 ...
 * Insert the method's description here.
 * Creation date: (8/17/2000 7:00:41 PM)
 * @param newSubject org.javaldap.server.syntax.DirectoryString
 */
public void setSubject(org.javaldap.server.syntax.DirectoryString newSubject) 
		throws org.javaldap.server.util.InvalidDNException {
	switch (getSubjectType()) {
		case SUBJECT_ROLE:
		case SUBJECT_GROUP:
		case SUBJECT_SUBTREE:
			subject = org.javaldap.server.util.DNUtility.getInstance().normalize(newSubject);
			break;
		default:
			subject = newSubject;
			break;
